...
flag = 0
char_temp = ''
con = psycopg2.connect(dbname = 'check', user = 'postgres', password = 'generic369', host = "127.0.0.1", port = "5432")
...
if (url[0:4] != "http") :
	...
	name_result = face_reco("C:\Work\Practice\Application\People", "img.jpg", -1, 0.6, False)
...
while (flag == 0) :
	...
	if (answer == "Yes") or (answer == "yes") or (answer == "y") :
		...
		flag = 1
		print('Does this person ever added in directory?')
		answer = input()
		if (answer == "No") or (answer == "no") or (answer == "n") :
			print('So, it\'s an unknown person\nPlease, enter the Name you would like to assign to this image')
			name_result = input()
			os.remove("img.jpg")
			name_result = "People\\" + name_result
			cur.execute("INSERT INTO IMAGE_IDENTIFY (URL,NAME) VALUES ('%s','%s');""" % (url, name_result[7::]))
			...
			print('Sorry, i can\'t recognize this one\nPlease, enter the Name of this person, so i can add it to the directory')
			name_result = input()
			os.remove("img.jpg")
			name_result = "People\\" + name_result + str(random.randint(1, 100))
			cur.execute("INSERT INTO IMAGE_IDENTIFY (URL,NAME) VALUES ('%s','%s');""" % (url, name_result[7::]))
			name_result = name_result + ".jpg"
			out = open(name_result, "wb")
			out.write(img)
